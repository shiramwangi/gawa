...
import { nyamaMamaAPI } from '../api/nyamaMamaAPI';
import FoodItemCard from './shared/FoodItemCard';

const NyamaMamaMenuPage = ({ setCurrentPage, setSelectedFood, setSelectedRestaurant }) => {
  ...
  useEffect(() => {
    const fetchMenuData = async () => {
      try {
        setLoading(true);
        const data = await nyamaMamaAPI.getFullMenu();
        setMenuData(data);
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-orange-400">Preparing Nyama Mama's African fusion menu...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-orange-400 mb-2">Oops! Something went wrong</h2>
  ...
  if (!menuData) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <p className="text-orange-400">No menu data available.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      {/* African Fusion Header */}
      <div className="bg-gray-700 border-b border-gray-600">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="text-center">
            <h3 className="text-lg font-semibold text-white mb-2">🌍 African Fusion Beef Cuisine</h3>
            <p className="text-gray-300 text-sm">Modern twists on traditional beef dishes with contemporary African flair!</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search Nyama Mama menu..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full px-4 py-3 pl-12 bg-gray-700 border-0 rounded-lg focus:ring-2 focus:ring-orange-500 focus:bg-gray-600 transition-all duration-300 text-white placeholder-gray-400"
                />
      ...
      <div className="bg-gray-800 border-t border-gray-700 mt-12">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="text-center">
            <h3 className="text-lg font-semibold text-orange-400 mb-2">About {menuData.restaurant.name}</h3>
            <p className="text-gray-300 mb-4">
              {menuData.restaurant.description || 'Modern African fusion cuisine with contemporary flair, celebrating Kenya\'s rich culinary heritage since 2010'}
            </p>
            
            {/* Restaurant stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">2010</div>
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">{menuData.restaurant.rating}</div>
                <div className="text-sm text-gray-400">Rating</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">30-45</div>
            ...
            {menuData.restaurant.specialties && Array.isArray(menuData.restaurant.specialties) && menuData.restaurant.specialties.length > 0 && (
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-orange-400 mb-2">Our Specialties:</h4>
                <div className="flex flex-wrap justify-center gap-2">
                  {menuData.restaurant.specialties.map((specialty, index) => (
                    <span key={index} className="bg-gray-700 text-orange-400 px-3 py-1 rounded-full text-xs">
                      {specialty}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-6">
                ...
                <div className="space-y-3">
                  {menuData.customization.options.doneness && (
                    <div>
                      <h5 className="text-xs font-medium text-orange-600 mb-1">Doneness Options:</h5>
                  ...
                  {menuData.customization.options.cuts && (
                    <div>
                      <h5 className="text-xs font-medium text-orange-600 mb-1">Cut Options:</h5>
                      <div className="flex flex-wrap justify-center gap-1">
                        {menuData.customization.options.cuts.map((option, index) => (
...
export default NyamaMamaMenuPage;
