...
import papaJohnsAPI from '../api/papaJohnsAPI';
import FoodItemCard from './shared/FoodItemCard';

const PapaJohnsMenuPage = ({ setCurrentPage, setSelectedFood, setSelectedRestaurant }) => {
  ...
  useEffect(() => {
    const fetchMenuData = async () => {
      try {
        setLoading(true);
        const data = await papaJohnsAPI.getFullMenu();
        setMenuData(data);
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-orange-400">Preparing Papa John's delicious menu...</p>
        </div>
      </div>
    );
  }
  ...
  if (!menuData) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-300">No menu data available.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search Papa John's menu..."
                  value={searchTerm}
      ...
      <div className="bg-gray-800 border-t border-gray-700 mt-12">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="text-center">
            <h3 className="text-lg font-semibold text-white mb-2">About {menuData.restaurant.name}</h3>
            <p className="text-gray-300 mb-4">{menuData.restaurant.description}</p>
            
            {/* Restaurant stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">{menuData.restaurant.founded}</div>
                <div className="text-sm text-gray-400">Founded</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">{menuData.restaurant.branches}</div>
                <div className="text-sm text-gray-400">Branches</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">4.3</div>
                <div className="text-sm text-gray-400">Rating</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">25-35</div>
            ...
            {menuData.restaurant.specialties && (
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-gray-300 mb-2">Our Specialties:</h4>
            ...
            <div className="flex justify-center space-x-6 text-sm text-gray-400">
              ...
              <span>🌐 <a href={menuData.restaurant.website} target="_blank" rel="noopener noreferrer" className="text-orange-400 hover:underline">Website</a></span>
            </div>

            {/* Popular locations */}
            {menuData.locations && menuData.locations.length > 0 && (
              <div className="mt-6">
                <h4 className="text-sm font-semibold text-gray-300 mb-2">Our Locations:</h4>
                <div className="flex flex-wrap justify-center gap-2">
            ...
            {menuData.deals && menuData.deals.length > 0 && (
              <div className="mt-6">
                <h4 className="text-sm font-semibold text-gray-300 mb-2">Current Deals:</h4>
                <div className="flex flex-wrap justify-center gap-2">
                  {menuData.deals.map((deal, index) => (
                    <span key={index} className="bg-red-900 text-red-300 px-3 py-1 rounded-full text-xs">
                      🔥 {deal}
                    </span>
                  ))}
                </div>
            ...
            {menuData.rewards && menuData.rewards.available && (
              <div className="mt-6">
                <h4 className="text-sm font-semibold text-gray-300 mb-2">{menuData.rewards.program} Benefits:</h4>
                <div className="flex flex-wrap justify-center gap-2">
                  {menuData.rewards.benefits.map((benefit, index) => (
                    <span key={index} className="bg-green-900 text-green-300 px-3 py-1 rounded-full text-xs">
                      🎁 {benefit}
                    </span>
                  ))}
                </div>
...
export default PapaJohnsMenuPage;
